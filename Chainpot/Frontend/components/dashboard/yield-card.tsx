...
export function YieldCard({ isDarkMode }: YieldCardProps) {
  const { totalAccrued, prevInterest } = useGlobalStats();
  
  const totalInterestDisplay = formatUSDC(totalAccrued);
  const prevInterestDisplay = formatUSDC(prevInterest);
      ...
      <div className={`border-3 border-black rounded-3xl p-8 backdrop-blur transition-colors space-y-6 ${isDarkMode ? 'bg-white/5' : 'bg-white/90'} `}>
        <div className="flex items-start justify-between">
          <div className="space-y-2">
            <h3 className={`text-sm font-semibold ${isDarkMode ? 'text-white/70' : 'text-black/70'}`}>Platform Yield Earned</h3>
            <p className={`text-4xl font-bold ${isDarkMode ? 'text-white' : 'text-black'}`}>{totalInterestDisplay}</p>
            <p className={`text-sm ${isDarkMode ? 'text-white/60' : 'text-black/60'}`}>Across all Business ROSCAs</p>
          </div>
        ...
        <div className={`flex justify-between items-center pt-4 border-t ${isDarkMode ? 'border-white/10' : 'border-black/10'}`}>
          <div className="space-y-1">
            <p className={`text-xs ${isDarkMode ? 'text-white/70' : 'text-black/70'}`}>Previous 30 days</p>
            <p className={`text-lg font-bold ${isDarkMode ? 'text-white' : 'text-black'}`}>{prevInterestDisplay}</p>
            <p className={`text-xs ${isDarkMode ? 'text-emerald-400' : 'text-green-600'}`}>From completed cycles</p>
          </div>
        </div>
      </div>
    </section>

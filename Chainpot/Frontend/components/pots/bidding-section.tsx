...
import { useLowestBid, useBidOf, usePlaceBid } from '@/hooks/useAuctionEngine';
...
export function BiddingSection({ potId, isDarkMode }: BiddingSectionProps) {
  ...
  const { data: potInfo, isLoading: potLoading } = useGetPot("auction", potIdBig);
  ...
  const { data: cycleInfo, isLoading: cycleLoading } = useGetCycle("auction", potIdBig, currentCycleIdx);
  const cycleStatus = cycleInfo ? (cycleInfo as any)[5] : 0;
  const paymentDeadline = cycleInfo ? Number((cycleInfo as any)[1] || 0) : 0;
  const biddingDeadline = cycleInfo ? Number((cycleInfo as any)[2] || 0) : 0;
  
  const isCycleActive = cycleStatus === 1;

  const { data: lowestBidRaw } = useLowestBid(potIdBig, currentCycleIdx);
  const lowestBid = BigInt(lowestBidRaw || 0);

  const { data: myBidRaw, refetch: refetchBid } = useBidOf(potIdBig, currentCycleIdx, address);
  const myBid = BigInt(myBidRaw || 0);

  const { data: hasPaidRaw, refetch: refetchPaid } = usePaidForCycle("auction", potIdBig, currentCycleIdx, address);
  ...
  const { payForCycle, isPending: isPayPending, isConfirming: isPayConfirming, isConfirmed: isPayConfirmed, error: payError } = usePayForCycle("auction");
  ...
  useEffect(() => {
    if (isBidConfirmed) {
      setTimeout(refetchBid, 1000);
      setBidAmount('');
    }
  }, [isBidConfirmed, refetchBid]);
  ...
  const handleBid = async () => {
    if (bidAmountBig >= lowestBid && lowestBid > BigInt(0)) {
      alert("Your bid must be lower than the current lowest bid!");
      return;
    }
    placeBid(potIdBig, bidAmountBig);
  };
  ...
  if (!isCycleActive) return null; // Only show when active

  return (
    <section className="mb-8">
      <h2 className="text-3xl font-bold mb-6">Auction: Place Your Discount Bid</h2>
      <div className={`p-8 rounded-2xl border-3 border-black ${isDarkMode ? 'bg-white/5' : 'bg-white/90'}`}>
        ...
        <div className="mb-6 flex items-center justify-center gap-4">
          ...
          <div className={`flex items-center gap-2 ${step >= 3 ? 'opacity-100' : 'opacity-50'}`}>
            <div className={`w-8 h-8 rounded-full flex items-center justify-center font-bold ${step === 3 ? 'bg-black text-white' : 'bg-gray-300'}`}>3</div>
            <span>Place Bid</span>
          </div>
        </div>
        ...
        <div className="flex flex-col md:flex-row gap-6 items-end">
          {hasPaid ? (
            <div className="flex-1">
              <label className="block text-sm font-bold mb-2">Discount Bid Amount (USDC)</label>
              <input
                type="number"
                value={bidAmount}
                onChange={(e) => setBidAmount(e.target.value)}
                placeholder={`Current Lowest: ${lowestBid > 0 ? formatUSD(lowestBid) : 'None'}`}
                className={`w-full px-4 py-3 rounded-lg border-2 border-black ${isDarkMode ? 'bg-white/5 text-white' : 'bg-white text-black'}`}
              />
              <CountdownTimer deadline={biddingDeadline} label="Bidding Ends In" isDarkMode={isDarkMode} />
              <p className="text-xs mt-2 text-gray-500">
                In V4, you bid the <b>discount</b> you are willing to accept. The lowest bid wins (takes the pot minus the bid).
              </p>
            </div>
          ) : (
            <div className="flex-1 p-4 rounded-lg bg-black/5">
              ...
              {hasPaid && <p className="text-green-500 mt-2 font-bold">You have paid for this cycle.</p>}
            </div>
          )}

          <button
            onClick={step === 1 ? handleApprove : step === 2 ? handlePay : handleBid}
            disabled={isApprovePending || isApproveConfirming || isPayPending || isPayConfirming || isBidPending || isBidConfirming}
            className="px-8 py-3 rounded-full font-bold bg-black text-white hover:bg-black/80 disabled:opacity-50"
          >
            {step === 1 ? 'Approve' : step === 2 ? 'Pay' : 'Bid'}
          </button>
        </div>

        {(approveError || payError || bidError) && (
          <p className="mt-4 text-red-500 text-sm">{(approveError || payError || bidError)?.message}</p>
        )}
      </div>
    </section>
  );

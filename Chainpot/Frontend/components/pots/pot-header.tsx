...
export function PotHeader({ potId, engine, isDarkMode }: PotHeaderProps) {
  ...
  const getStatusText = () => {
    if (status <= 1) return "Open (Waiting for members)";
    ...
    <section className="mb-8">
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6 mb-6">
        ...
        <div className="flex gap-3 flex-wrap">
          ...
          {isCreator && status === 2 && (cycleStatus === 0 || cycleStatus === 3) && completedCycles < expectedMembers && (
          ...
          {isCreator && status === 2 && cycleStatus === 1 && (
          ...
          {isCreator && status === 2 && engine === 'auction' && cycleStatus === 1 && (
          ...
          {isCreator && status === 2 && engine === 'circle' && cycleStatus === 1 && (
      ...
      <div className={`p-4 rounded-xl border-2 border-black ${isDarkMode ? 'bg-white/5' : 'bg-white/90'}`}>
        <div className="flex items-center justify-between flex-wrap gap-4">
          <div className="flex items-center gap-2">
            <span className={`w-3 h-3 rounded-full ${status === 2 ? 'bg-green-500' : 'bg-yellow-500'}`} />
